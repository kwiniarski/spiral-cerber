...
function authFactory(config) {
  ...
  function calculateRequestHash(request, token) {
    ...
    return util.hmacHash('sha1', token, data.join('\u000A'));
  }
  ...
  function hasValidTimestamp(ts) {
    return Math.abs(options.clock() - ts) <= options.validityTime;
  }
  ...
  var api = {
    ...
    authorizeRequest: function (request, cb) {
      ...
      if (!credentials) {
        ...
        identify(credentials.id, function (err, user) {

          if (!user) {
            cb(unauthorized('User authorization required', request, user));
          }

          else if (!user.nonce(credentials.nonce)) {
            cb(unauthorized('Request nonce is invalid', request, options));
          }

          else if (credentials.hash !== calculateRequestHash(request, user[apiKey])) {
            cb(unauthorized('Invalid request hash', request, user));
          }
